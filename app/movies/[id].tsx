    ...
    <View className="bg-primary flex-1">
      <ScrollView contentContainerStyle={{paddingBottom: 80}}>
        ...
        <View className="flex-col items-start justify-center mt-5 px-5" >
          ...
          <View className="flex-row items-center bg-dark-100 px-2 py-1 rounded-md cap-x-1 mt-2">
          ...
          <View className="flex flex-row justify-between w-1/2">
            <MovieInfo label="Budget" value={`$${(Math.round((movie?.budget ?? 0) / 500_000) * 0.5).toFixed(1)} million`} />
            <MovieInfo label="Revenue" value={`$${(Math.round((movie?.revenue ?? 0) / 500_000) * 0.5).toFixed(1)} million`}/>
          </View>
